...
import com.zero.map.ArrayAdapter;
import com.zero.map.BMapUtil;
import com.zero.map.CustomItem;
import com.zero.map.CustomIconOverlay;
import com.zero.map.CustomTextOverlay;
	...
	public void initLocationService() {
		...
		locationClient.registerLocationListener(new BDLocationListener() {
			...
			public void onReceiveLocation(BDLocation location) {
				...
				//
				locationClient.stop();
				Toast.makeText(jxauMapAct.this, "定位成功", Toast.LENGTH_SHORT)
						.show();
...
//		mBtnNext.setVisibility(View.INVISIBLE);
		...
		if (start != null && !start.equals("当前位置")) {
			...
			Toast.makeText(jxauMapAct.this, "请输入起点", Toast.LENGTH_SHORT).show();
			return;
		}
		...
		if (end != null && !end.equals("地图上的点")) {
			...
			Toast.makeText(jxauMapAct.this, "请输入终点或在地图上选点", Toast.LENGTH_LONG)
					.show();
			return;
		}
	...
	public void dealWithMKSearchData(ArrayList<MKPoiInfo> stPois,
		...
		if (!editStart.getText().toString().equals(STATU_START)) {
			...
			Toast.makeText(jxauMapAct.this, "无法找到该地址!", Toast.LENGTH_LONG)
					.show();
		}
	}
	...
	MKSearchListener myMKSearchListener = new MKSearchListener() {
		...
		public void onGetDrivingRouteResult(MKDrivingRouteResult res, int error) {
			...
			// 清除其他图层
			BMapUtil.removeNotCustomOverlay(mMapView);
			// 添加路线图层
...
//			}
		...
		case R.id.search_btn:
			...
			if(place == null){
				...
				b[0]=BMapUtil.getBitmapFromView(popleftText);
				b[1]=BMapUtil.getBitmapFromView(popupText);
				b[2]=BMapUtil.getBitmapFromView(poprightText);
